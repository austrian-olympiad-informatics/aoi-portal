...
class Contest(Base):
    ...
    # NOTE: Contest timing (start/stop, analysis mode, per_user_time) moved to
    # the Group model upstream ("user groups" feature). See Contest.main_group
    # and Participation.group; the phase() helper now lives on Group.
    ...
    # Main group (id and object) of this contest: the default timing group.
    main_group_id: int | None = Column(
        ...
        nullable=True,
        index=True)
    main_group: Mapped["Group | None"] = relationship(
        "Group",
