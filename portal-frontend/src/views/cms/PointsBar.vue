  ...
  >
    ...
    <template v-else>
      <div
        ...
        :key="index"
        />
      <p class="progress-value" :class="[subtaskMoreThanHalfClass]">{{ scoreShow }}</p>
    </template>
  </div>
</template>
...
export default class PointsBar extends Vue {
  ...
  get subtaskData() {
    ...
    for (const st of this.subtasks) {
      ...
      if (posWidth > 0 && (
        (posEnd >= 45 && posEnd <= 55)) || (posStart >= 45 && posStart <= 55)
  ...
  get isSubtasks(): boolean { return this.subtasks !== null; }
  ...
  get scoreShow(): string {
    ...
    if (this.showMaxScore) {
      const b = parseFloat(this.maxScore.toFixed(this.scorePrecision)).toString()
      return `${a} / ${b}`;
    }
    return a;
  }
